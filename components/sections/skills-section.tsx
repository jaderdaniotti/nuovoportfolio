    ...
    >
      <div className="mx-auto flex h-full w-full max-w-5xl flex-col justify-center gap-6">
        ...
        >
          ...
          <h2 className="max-w-3xl text-[clamp(1.9rem,5.1vh,3rem)] font-bold tracking-tight text-zinc-900 dark:text-zinc-100">
        ...
        <div className="grid w-full grid-cols-1 gap-6 md:grid-cols-2 md:gap-8">
          {skillGroups.map((group, i) => (
            ...
            >
              <h3 className="border-b border-zinc-100 pb-3 text-xl font-semibold tracking-tight text-zinc-800 dark:border-zinc-800 dark:text-zinc-200">
                {group.title}
              </h3>
              <div className="flex flex-wrap gap-2">
                {group.items.map((item, j) => (
                  <motion.span
                    ...
                    className="inline-flex items-center rounded-full bg-zinc-100 px-3 py-1.5 text-xs font-medium text-zinc-700 transition hover:bg-zinc-200 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700 md:text-sm"
                  >
                    {item}
                  </motion.span>
                ))}
