...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    const getWebGLContext = (el: HTMLCanvasElement) => {
      const params = {
        alpha: true,
        depth: false,
        stencil: false,
      ...
      let gl = el.getContext("webgl2", params) as WebGL2RenderingContext | null;
      if (!gl) {
        gl = (el.getContext("webgl", params) ||
          el.getContext("experimental-webgl", params)) as WebGL2RenderingContext | null;
      }
      if (!gl) return null;
    ...
    const addKeywords = (source: string, keywords: string[] | null) => {
    ...
    class Material {
      ...
      setKeywords(keywords: string[]) {
        const hash = keywords.reduce((acc, kw) => acc + kw.length * 31, 0);
